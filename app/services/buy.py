...
def execute_buy(symbol: str) -> dict:
    ...
    # 2) TP/SL 전용 무한 백오프 (서버 과부하 시 반드시 성공시킬 때까지)
    ...
    try:
        ...
        # precision 계산 및 자본 기반 allocation
        ...
        def _monitor():
            ...
            while True:
                ...
                if tp1_id and tp1_active and tp1_id not in open_ids:
                    ...
                    logger.info(
                        f"Moved SL to +0.1% @ {new_sl_p} for qty {new_qty}"
                    )
                    tp1_active = False
                ...
                if tp2_id and tp2_active and tp2_id not in open_ids:
                    ...
                    current_sl_id = new_sl["orderId"] if new_sl else current_sl_id
                    logger.info(
                        f"Moved SL to +0.5% @ {new_sl_p} for qty {new_qty}"
                    )
                    tp2_active = False
